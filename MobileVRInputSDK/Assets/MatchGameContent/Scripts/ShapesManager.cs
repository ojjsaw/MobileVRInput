...
public class ShapesManager : MonoBehaviour
{
    ...
    private Behaviour component = null;
    ...
    void Start()
    {
        ...
        MVRInputManager.OnTouchSwipe += MVRInputManager_OnTouchSwipe;
    }

    private void MVRInputManager_OnTouchSwipe(MVRSide screenSide, MVRSide swipeDirection)
    {

        if (hitGo != null && state == GameState.None)
        {
            ...
            {
                case MVRSide.LEFT:
                    ray = new Ray(hitGo.transform.position, hitGo.transform.right * -1f);
                    break;
                case MVRSide.RIGHT:
                    ray = new Ray(hitGo.transform.position, hitGo.transform.right);
                    break;
                case MVRSide.UP:
                    ray = new Ray(hitGo.transform.position, hitGo.transform.up);
                    break;
                case MVRSide.DOWN:
                    ray = new Ray(hitGo.transform.position, hitGo.transform.up * -1f);
                    break;
            }
            ...
            if (Physics.Raycast(ray, out hit, 5.0f) && hitGo != hit.collider.gameObject)
            {
    ...
    void Update()
    {
        ...
        if (state == GameState.None)
        {
            ...
            if (Physics.Raycast(ray, out hit, 200.0f))
            {
                ...
                if (hit.collider.gameObject.GetComponent("Halo") == null)
                {
                    hitGo = null;
                    return;
                }
